...
export const TestimonialsSection: React.FC = () => {
  ...
  const filtered =
    filterCategory === 'all'
      ? testimonialsList
      : testimonialsList.filter((t) => t.category === filterCategory);

  return (
    <section id="testimonials" className="py-20 bg-[#EFEDE7] border-t border-[#D9D7D0] relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Section Header with Google Rating Badge and Scroll Reveal */}
        <ScrollReveal animation="fade-up" duration={650}>
          <div className="text-center max-w-3xl mx-auto mb-12">
            ...
            <div className="inline-flex items-center gap-3 bg-white px-4 py-2.5 rounded-full border border-[#D9D7D0] shadow-sm mt-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {filtered.map((t, idx) => (
            <ScrollReveal
              ...
              className="h-full"
            >
              <div className="bg-white rounded-3xl p-6 sm:p-8 border border-[#D9D7D0] shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 relative flex flex-col justify-between group h-full">
                <Quote className="w-12 h-12 text-[#D8E8D4] absolute top-6 right-6 pointer-events-none group-hover:text-[#B5CAB0] transition-colors" />
                ...
                <div className="space-y-4">
                  ...
                  <p className="text-sm sm:text-base text-[#2C2B29] leading-relaxed italic relative z-10 font-normal">
                    "{t.comment}"
                  </p>
                </div>
                ...
                <div className="pt-6 mt-4 border-t border-[#EFEDE7] flex items-center justify-between">
                  <div className="flex items-center gap-3.5">
                    {t.avatarImageUrl ? (
                      ...
                      <div
                        className={`w-12 h-12 rounded-full bg-gradient-to-br ${t.bgGradient} text-white font-bold font-display flex items-center justify-center text-sm shadow-sm flex-shrink-0`}
                      >
                        {t.avatarText}
                      </div>
                    )}
                    ...
                    <div>
                      <h4 className="font-display font-bold text-base sm:text-lg text-[#1E1D1B] leading-none">
                        {t.name}
                      </h4>
                      <div className="text-xs text-[#5A5854] mt-1">{t.role}</div>
                      <div className="text-[11px] text-[#3D6338] font-bold mt-0.5">
                        {t.enrolledClass}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="text-right hidden sm:block">
                    <span className="text-[11px] px-3 py-1 bg-[#F7F5F0] text-[#3D6338] font-bold rounded-full border border-[#D9D7D0]">
                      {t.yearsWithStudio}
                    </span>
                  </div>
                </div>
