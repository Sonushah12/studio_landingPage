...
import { Check, Sparkles, Shield, Gift, ArrowRight, Tag, HelpCircle } from 'lucide-react';
...
export const PricingCalculator: React.FC<PricingCalculatorProps> = ({ onOpenTrialModal }) => {
  ...
  const baseMonthly = classesPerWeek === 2 ? 2200 : classesPerWeek === 3 ? 3000 : 4500;

  // Multiplier for billing cycle
  const months = billingCycle === 'monthly' ? 1 : billingCycle === 'quarterly' ? 3 : 12;
  const discountPercent = billingCycle === 'monthly' ? 0 : billingCycle === 'quarterly' ? 15 : 25;

  // Add-ons
  const recitalCost = includeRecitalPass ? (billingCycle === 'annual' ? 0 : 1500) : 0; // free on annual
  const privateCost = includePrivateCoaching ? 1800 * (billingCycle === 'monthly' ? 1 : billingCycle === 'quarterly' ? 2 : 6) : 0;

  // Subtotal calculation
  const rawTuition = baseMonthly * months;
  const discountedTuition = Math.round(rawTuition * (1 - discountPercent / 100));
  const siblingSavings = isSiblingDiscount ? Math.round(discountedTuition * 0.1) : 0;
  const finalTotal = discountedTuition - siblingSavings + recitalCost + privateCost;
  const savingsTotal = rawTuition - discountedTuition + (includeRecitalPass && billingCycle === 'annual' ? 2500 : 0) + siblingSavings;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12">
          ...
          <p className="text-[#5A5854] text-sm mt-3">
            No hidden admission costs. Build your tailored dance pass below and enjoy complimentary trial guarantee.
          </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-7 bg-white p-6 sm:p-8 rounded-3xl border border-[#D9D7D0] shadow-sm space-y-6">
            {/* Step 1: Frequency */}
            <div>
          ...
          <div className="lg:col-span-5 bg-gradient-to-b from-[#1E1D1B] to-[#2C2B29] text-white p-6 sm:p-8 rounded-3xl shadow-xl space-y-6">
            ...
            <button
              ...
              className="w-full py-4 bg-[#3D6338] hover:bg-[#4E7D47] text-white rounded-2xl font-bold text-xs uppercase tracking-wider transition shadow-lg flex items-center justify-center gap-2 cursor-pointer"
            >
